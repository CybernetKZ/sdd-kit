...
Статус: **принято**. Дата: 2026-08-03 (grill-сессия №2 по дедупликации, текстовый аудит 4 экосистем + интеграция PROMPT_AUDIT_SDD_KIT.md).
...
6. **C2 (кит перестаёт копировать `database-reviewer`)** — только когда плагин code-conventions станет обязательным для команды; до этого задвоение безвредно (после слияния текст один).
