...
## Фаза 1. ~~Включить главный гейт~~ -> ОТЛОЖЕНА СОЗНАТЕЛЬНО (ADR-0015, 2026-08-01)

Решение гриль-сессии 2026-08-01: enforcement остаётся **advisory-first** - sdd-check, tbd-gates и pre-commit являются сигналами, а не принуждением. Архитектура ADR-0003 (server-side CI = главный гейт) сохраняется как целевая; её активация - осознанно отложенное решение, а не долг. Пункты ниже выполняются, когда/если команда решит включить принуждение:

- [ ] 1.1 (отложено) Закоммитить `sdd-ci.yml` в целевые репо - файл раскатан фазой 7, но untracked (в WBN точно).
- [ ] 1.2 (отложено) Branch protection на `dev`: required check `sdd-gate`, запрет прямых пушей.
- [ ] 1.3 (отложено) Protection в `cybernet-specs`; после этого убрать `push: main` из `templates/store-ci.yml` (2.8).

**Проверка** (при активации): тестовый PR с невалидной спекой падает на CI; прямой push в dev отклоняется сервером.
...
- [x] 7.1 Для каждого целевого репо: удалить устаревшие файлы (`format-py.js`, старые ревьюеры, `settings-living-spec.json` если есть) и перезапустить `bootstrap.sh` для установки новых версий (или точечно скопировать изменённые шаблоны).
- [x] 7.2 Обновить `.claude/settings.json` в репо, где он уже существовал до этого обновления (bootstrap его не тронет - merge руками).
- [x] 7.3 Прогнать `make sdd-doctor` в каждом репо - ноль fail.
...
**Фазы 2-7 выполнены полностью** (все чекбоксы выше). Сверх плана по ходу исполнения:

- **Фикс `.cjs`** (критический): в ESM-репо (`"type": "module"`, web-frontend-new) CommonJS-хуки падали в рантайме (`require is not defined`) - block-no-verify и spec-guard были декоративными. Хуки переименованы в `.cjs` (шаблоны + settings.json + install.sh + ci.yml + uninstall.sh чистит оба варианта) и раскатаны по всем 6 репо; self-test зелёный везде.
- **6.1 отменён**: механизм "profile payload" ожил (`profiles/voice-agent-constructor-backend/`), шаг 2b сохранён.
- **uninstall.sh**: unattended-режим молча снимал регистрацию реального store - зафиксировано в DEFECTS_BACKLOG (нужен guard).
- **Раскатка (фаза 7)**: 6/6 репо, `make sdd-doctor` - 0 fail во всех; ничего не закоммичено в целевых репо (на ревью владельцу).

**Решения гриль-сессии 2026-08-01** (детали - ADR-0015, ADR-0016):

| # | Решение |
|---|---|
| D1 | Enforcement - сознательно advisory; активация ADR-0003 отложена (фаза 1 выше) |
| D2 | Feature flags - dormant/on-demand: реестр остаётся, но ни один шаг процесса не требует флага; механизм FLAG_X=1 на stage/prod - открытый вопрос |
| D3 | Тесты до кода пишет агент `test-author` (новый в ките); QA-владение - target |
| D4 | Скоуп - мульти-стек; frontend-профиль (frontend-reviewer, TS-флаги) - бэклог, низкий приоритет |
| D5 | conversation_flow вне скоупа кита: `.spec-guard-paths` опустошён, профиль закомментирован |
| D6 | Store живёт: потребитель - агенты (кросс-репо контекст), не машинные гейты |
