...
B1-B4 OK; `repo-auditor` -> `templates/agents/` + kit_manifest OK; кэши вычищены OK; сверх плана - P0-2 (planner получил Write/Edit), P0-3 (plan-griller переписан в one-shot, 48 строк), P0-1-пин (`$OPENSPEC` больше не падает на глобальный непиненный бинарь - всегда `npx -y @fission-ai/openspec@1.7.0`). Локальные проверки зелёные (bash -n, node --check, py_compile); закоммитить + прогнать self-CI (bootstrap-smoke) - Даниил.

### Фаза 1 - фундамент в conversation_flow (~полдня) - **ВЫПОЛНЕНА 2026-08-03, не закоммичена**
1. OK patch49: diff проверен - все 21 "изменение" это перенос строк (IDE-переформат), семантика не тронута; коммитить как есть безопасно.
2. OK `openspec init` на пиненном 1.7.0 (config.yaml создан; store `references:` - в фазе 2). По ADR-0020 п.8: `commands/opsx/` удалены, 6 openspec-скиллов проштампованы `disable-model-invocation: true`.
3. OK Kit-дроп готов к коммиту (коммитит Даниил); по P0-7 из CF удалены мёртвые `feature-flow`/`incident-flow` (оговорка: `--refresh` их вернёт, пока не сделано профильное гейтирование манифеста - P0-4); копии planner/plan-griller в CF синхронизированы с починенными шаблонами фазы 0.
4. OK AGENTS.md канонический 246 строк (всё содержимое старых файлов сохранено), `CLAUDE.md` -> симлинк, USA v4 -> `docs/DEPLOY_USA.md` (311 строк, дословно). `make sdd-check` зелёный и не вакуумный (config.yaml существует; specs/ наполнится в фазе 3).
...
- OK `add-cf-dialer-integration-api` - 26 Requirements / 81 Scenarios, якоря сверены с кодом; **12 расхождений записано** (топ: webhook `call.status` = `ended|active` против REST `queued|ringing|ongoing|ended|failed` - одно имя поля, два словаря; "must be HTTPS" из гайда не энфорсится - `allow_http` no-op; 5 недокументированных полей объекта звонка).
- OK `add-cf-rag-contract` - 13 Requirements / 30 Scenarios; **13 расхождений записано** (топ: лимиты SearchRequest не энфорсятся -> 422 без ретрая и RAG молча выключен; деградация ловит только RagError, ValidationError парсинга роняет тёрн; литеральный адрес в §11.9; расхождение имён S3-бакета).
- OK В CF - `references: cybernet-specs` в config.yaml (формат как у WBN), `sdd-check` зелёный. Обновление SOURCES.md/README-таблицы - задачи в tasks.md обоих changes (на apply-этапе, с пересчётом итогов из-за параллельности).
...
- OK Волна 1: `flow-schema` 34 Req/37 Sc (верификация: 28 CONFIRMED, 0 MISMATCH кода, 5 пробелов полноты закрыты fixup'ом), `flow-execution` 23 Req/37 Sc (верификация opus: 3 MISMATCH кода + 2 завышения спеки + 8 пропусков - всё исправлено fixup'ом, дефекты E1-E5 в DEFECTS_CF.md). Оба гейта (`validate --all --strict`, spec-lint strict) зелёные. Конвейер: майнер(sonnet) -> верификатор(скептик) -> fixup(sonnet) - работает, верификатор обязателен: оба майнера заявляли "расхождений нет".
- OK Архив: все 83 патча + 4 безномерных -> 87 `openspec/changes/archive/tz-*` (proposal.md + tasks.md, оригиналы не тронуты). Аномалии в отчётах партий (у ТЗ №45/46 тестов нет вовсе).
- 🔄 Волна 2 запущена: майнеры `public-api-v1` (со сверкой против store-контракта), `webhooks`, `persistence-versioning`.
По завершении: слепок `openspec/**` копируется в `profiles/conversation_flow/` (решение 2); в DOCUMENTATION.md шапку - строка "канон поведения - openspec/specs/, документ ведётся параллельно (ADR-0019)"; `docs/patches/README.md` - **не "архив закрыт"**: на ветке `main` патчи создаются по-прежнему (параллельная разработка старым процессом), новым процессом файл здесь не создаётся, отказ от патчей - при cutover; отставание ловится `tools/cf/main-drift.sh`.
...
## 4а. Параллельная разработка на `main`: как ловим дрейф

Миграция идёт на `test-sdd-kit`, а разработка не останавливается: на `main`
продолжают появляться патчи в `docs/patches/`, записи §17 и код. Дрейф считает
`tools/cf/main-drift.sh` (из корня conversation_flow, только чтение, exit 1 =
дрейф есть - годится как CI-сигнал, `--self-check` проверяет сам скрипт): он
печатает merge-base и расхождение ветвей, ТЗ на `main` без `tz-*` в
`openspec/changes/` и `changes/archive/` (сверка по номеру), новые записи §17 и
смену версии документа, и таблицу "изменённый на `main` файл -> какие спеки
станут STALE" по `enforced:`-якорям `openspec/specs/*` (файлы без якорей
показываются отдельно - это пробелы покрытия). Прогон 2026-08-03 на
merge-base `4653f63f`: 8 ТЗ (№92-99, требования на исправление наших дефектов,
...
Даниил), и ожидаемый мерж-конфликт в `docs/DOCUMENTATION.md` (шапка с блоком
про канон openspec с нашей стороны против тела и §17 со стороны `main`),
который разрешается объединением, а не выбором стороны.

**Мерж отложен (2026-08-03, решение Даниила: "в ближайшие дни не будет").**
Следствия, зафиксированные намеренно, чтобы промежуточное состояние не читалось
как поломка: на `main` продолжают появляться патчи (уже №92-99 и далее), в свежем
клоне `spec-lint` показывает `FRESH=5 / STALE=15` (спеки с нашей ветки против кода
`main`, где ТЗ №92/№93 реализованы), `AGENTS.md`/`CLAUDE.md` остаются двумя файлами
с WARN в гейте, а `tz-092`/`tz-093` висят активными change'ами на уже сделанную
работу. Ре-верификацию спек и обновление маркеров до мержа **не делаем** - иначе
работа делается дважды; отставание отслеживаем только `main-drift.sh`. Архивировать
`tz-092`/`tz-093` тоже нельзя до мержа: архивация оторвала бы change от кода,
которого в нашей ветке ещё нет. Порядок неизменен: мерж -> drift -> архивация
реализованного -> ре-верификация -> маркеры.
