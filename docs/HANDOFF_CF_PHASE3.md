...
| voice-pipeline (fixup волны 3) | 17->31 | OK 4 MISMATCH исправлены, 7 Req добавлено |
...
1. OK ~~fixup `tenancy-auth`~~, OK ~~верификаторы `editor-ui` и `rag`~~, OK ~~fixup'ы волны 4 (`cost-analytics`, `monitoring`, `rag`, `editor-ui`; `timezones` правок не требовал)~~, OK ~~ре-верификация всех 7 STALE-спек~~ - см. раздел "Финал фазы 3" выше.
2. OK ~~Пробелы покрытия G1-G9~~ - **ВСЕ ЗАКРЫТЫ** 2026-08-03, см. раздел "Закрытие пробелов" ниже.
...
## Аудит чистой установки (2026-08-03): 12 дефектов кита, все починены

Даниил сделал чистый `git clone` в `/home/octrow/dev/conversation_flow` (ветка `main`, HEAD "ТЗ №93: LIVING SPEC 1.91.0") и прогнал `install.sh`. Сравнение с эталоном `/home/octrow/cybernet/conversation_flow` (`test-sdd-kit`) вскрыло 12 дефектов (все починены). `openspec/` при этом побайтово идентичен (239/239), как и все 7 агентов, воркфлоу, pre-commit, `.mcp.json`, payload-скиллы - механизм сида работает как задумано, дефект в свежести того, что он приносит.
...
| OK 12 | `.spec-guard-paths`: профиль закомментирован, эталон несёт файл-из-одних-комментариев - поведение одинаковое, но замысел теряется | **ПОЧИНЕНО** 2026-08-03 (решение Даниила после того, как мерж отложился): `PROFILE_SPEC_GUARD_PATHS` в профиле теперь несёт блок-из-комментариев с девятью префиксами, закомментированными ВНУТРИ значения - файл создаётся при установке, отказ задокументирован в репозитории, фаза 5 включается раскомментированием. Ноль активных префиксов проверен, `spec-guard.cjs` на правку `api.py` выходит с 0, доктор говорит "has no path prefixes (comments only)" вместо "no .spec-guard-paths". Заодно переписан устаревший текст файла в эталоне: он утверждал "conversation_flow is out of sdd-kit scope", что ADR-0019 отменил |
...
> **Мерж отложен (решение Даниила 2026-08-03: "в ближайшие дни не будет").** Значит расчёт "мерж закроет само" верен по существу, но не скоро - и до тех пор в силе промежуточное состояние, которое надо помнить, чтобы не принять его за поломку:
> - на `main` и в любом свежем клоне `AGENTS.md` - 78 строк деплой-контекста, `CLAUDE.md` - настоящий 429-строчный контекст; `make sdd-check` даёт WARN, коммиты НЕ блокирует, доктор говорит "канон, похоже, перепутан". Это ожидаемо, чинить в ките нечего;
> - `spec-lint` в свежем клоне даёт `FRESH=5 / STALE=15` - спеки приехали с `test-sdd-kit`, код с `main` (ТЗ №92/№93 реализованы). Тоже ожидаемо; ре-верификация имеет смысл только после мержа, иначе маркеры придётся ставить дважды;
> - `.spec-guard-paths` в свежем клоне отсутствует -> `WARN: no .spec-guard-paths` вместо эталонного "comments only". Дефект 12 остаётся открытым дольше, чем предполагалось, поэтому решение "дождаться мержа" стоит пересмотреть: если ждать дни, дешевле научить профиль создавать файл-из-комментариев сразу (правка в несколько строк);
> - **`tz-092` и `tz-093` - активные change'ы на уже сделанную работу.** Пока мержа нет, они будут висеть активными и попадать в `openspec validate`/`spec-lint` как живые. Не архивировать их "на всякий случай": архивация до мержа оторвёт change от кода, который в нашей ветке ещё не появился. Правильный порядок остаётся: мерж -> `main-drift.sh` -> архивация реализованного -> ре-верификация -> маркеры.
