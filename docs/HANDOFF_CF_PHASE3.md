...
| ✅ 12 | `.spec-guard-paths`: профиль закомментирован, эталон несёт файл-из-одних-комментариев — поведение одинаковое, но замысел теряется | **ПОЧИНЕНО** 2026-08-03 (решение Даниила после того, как мерж отложился): `PROFILE_SPEC_GUARD_PATHS` в профиле теперь несёт блок-из-комментариев с девятью префиксами, закомментированными ВНУТРИ значения — файл создаётся при установке, отказ задокументирован в репозитории, фаза 5 включается раскомментированием. Ноль активных префиксов проверен, `spec-guard.cjs` на правку `api.py` выходит с 0, доктор говорит «has no path prefixes (comments only)» вместо «no .spec-guard-paths». Заодно переписан устаревший текст файла в эталоне: он утверждал «conversation_flow is out of sdd-kit scope», что ADR-0019 отменил |
