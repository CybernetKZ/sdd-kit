...
|---|---|---|---|
| ✅ G1 | **ЗАКРЫТ** 2026-08-03 → 10 Req в `call-control` (`transfer-dispatch`, `-cold-refer`, `-blind-invite`, `-warm-hold`, `-warm-briefing`, `-bridge-exit`, `-bridge-observer`, `-finalize`, `-failure-fallback`, `-agent-handoff`, `-timeline-event`) + инварианты `inv-briefing-privacy`, `inv-failure-not-disposition`. Транспорт перевода осознанно оставлен в `telephony`. Попутно вскрыты CC1, CC2. Исходный текст: transfer-рантайм (§8.9, ТЗ №28 cold/warm): hold, брифинг, бридж и наблюдение за ним, хендофф, запись после перевода | `livekit_agent/agent.py:1630-2009` (`_cold_transfer`/`_warm_transfer`/`handle_transfer`) | telephony делегировал в call-control (`telephony/spec.md:385-391`), call-control не взял — верификатор call-control подтвердил дыру |
| ✅ G2 | **ЗАКРЫТ** 2026-08-03 → 3 Req (`auto-end-sequence`, `auto-end-disposition`, `auto-end-disconnect-reason`); гейт ТЗ №21 не дублирован — ссылка на `cost-analytics`. Попутно вскрыты CC3, CC5. Исходный текст: пайплайн авто-завершения (§8.8 §6): `auto_end_call` → `end_reason` → `POST /api/calls/{room}/status` → `compute_disposition`/`AUTO_END_DISPOSITIONS` → гейт ТЗ №21 → `call_ended.disconnect_reason` | `livekit_agent/agent.py`, `analytics/` | верификатор call-control |
| ✅ G3 | **ЗАКРЫТ** 2026-08-03 → 3 Req (`hangup-fast-close`, `hangup-transfer-guard`, `hangup-web-channel`). Важное уточнение из кода: терминальный статус из этого пути НЕ уходит (идёт `ended` из `_report_ended`), поэтому disposition считается по транскрипту. Попутно вскрыт CC4. Исходный текст: быстрое завершение по брошенной трубке (§8.13 §3): `participant_disconnected` → событие `hangup` → немедленное закрытие джоба вместо ожидания `silence_timeout` | `livekit_agent/agent.py` | верификатор call-control (grep: `participant_disconnected` нет ни в одной спеке) |
| ✅ G4 | **ЗАКРЫТ** 2026-08-03 → 2 Req (`speech-tail-commit`, `speech-tail-suppression`) + инвариант `inv-tail-single-mechanism`; покрытие тестами здесь самое плотное из четырёх (6 тестов в `test_patch83.py`). Исходный текст: ТЗ №83, коммит хвоста речи (`commit_speech_tail`, `HANGUP_COMMIT_TIMEOUT`) | `livekit_agent/` | верификатор call-control |
| ✅ G5 | **ЗАКРЫТ** 2026-08-03 → 4 Req в `tts` (`voice-catalog-lifecycle`, `voice-catalog-tenancy`, `voice-bulk-replace`, `voice-check`). Тенантность справочника проверена: паттерна T2 здесь НЕТ, всё идёт через `_catalog_scoped`/`_org_visible`, как каталог LLM-моделей; выделенного регресс-теста на изоляцию `tts_voices` нет (есть аналог на `llm_models`) — пробел теста, не дефект. Исходный текст: справочник голосов TTS — CRUD/lifecycle (§8.14 §2): сид+trilateral merge, custom-запись/архив, массовая замена source→target (`drafts_only\|republish`), `api/tts_voices.py`, привилегированные `/v1/tts-voices/*` | `storage/tts_voice_presets.py`, `api/tts_voices.py` | верификатор tts (в flow-schema только форма поля) |
| ✅ G6 | **ЗАКРЫТ** 2026-08-03 → `tts.chunk-log`; попутно вскрыт CA6 (за потолком 500 теряется хвост, себестоимость занижается). Исходный текст: лог TTS-чанков звонка (§8.12 §1): `VoicePublisher.tts_chunks` (потолок 500), персист в `sessions.meta.tts_chunks`, отдача в `GET /api/sessions/{id}`, раскладка по репликам в транскрипте | `livekit_agent/publisher.py`, `api/voice.py`, `api/main.py` | верификатор tts |
...
| ✅ G11 | №86 | **ЗАКРЫТ** 2026-08-03 → `persistence-versioning.explicit-flow-addressing` / `.publish-attribution` / инвариант `.active-flag-vestigial` + `editor-ui.explicit-route-identity` / `.author-display-resolution`. Проверено по коду: легаси-роуты (`GET/PUT /api/flow`, `POST /api/flow/{publish,discard,validate,estimate}`, `POST /api/flows/{name}/open`) реально удалены (404/405), а `FlowRow.active` не читается и не пишется НИГДЕ вне объявления колонки (грep по `storage/`, `api/`, `livekit_agent/`, `copilot/` — 0 совпадений), `set_active`/`get_active`/`active_name` отсутствуют; `DROP COLUMN` осознанно вне scope (разрушающая миграция отложена). Исходно не покрыто: 4 решения из ~6: `created_by_user_id` публикации (грep по всем спекам — 0 совпадений, есть только старое `created_by` со значением «user»/«user+copilot»), резолюция отображения `{id, display_name}` в листингах и панели версий, identity в маршруте фронта `#/editor/<name>`, вестигиальность `flows.active` и удаление старых роутов (`GET/PUT /api/flow`, `POST .../open` как мутация) | `persistence-versioning`, `editor-ui` |
...
| ✅ G13 | №89 | **ЗАКРЫТ** 2026-08-03 → `editor-ui.transcript-relative-offset`, `.transcript-absolute-time-in-tooltip`, `.telephony-monitor-absolute-time` (исключение D6 подтверждено: `TelephonyPage.tsx` по-прежнему на `fmtTime`, `fmtOffset` там нет). Покрыто существующими `tests/test_patch89.py` (все 4 класса). Исходно не покрыто: **Полностью MISSING**: относительный офсет времени реплики — хелпер `fmtOffset`, цепочка якоря нуля (`talk_from → audio_started_at → started_at → min(spoke_at??ts)`), правило «абсолют только в тултипе», исключение монитора телефонии | `editor-ui` |
...
| CA6 | MEDIUM (тихая потеря денег) | код | Потолок лога TTS-чанков молча занижает себестоимость длинных звонков. `record_tts_chunk` (`livekit_agent/publisher.py:79-83`) при `len(tts_chunks) >= 500` просто перестаёт добавлять — сохраняются ПЕРВЫЕ 500 чанков, хвост теряется (не FIFO, не скользящее окно). А `_cost_summary` (`analytics/costs.py:403-407`) предпочитает `_tts_lines(tts_chunks, …)` всегда, когда `tts_chunks` непуст, и падает на беспотолочный `_tts_usage_line(usage["tts"], …)` только когда лог пуст ЦЕЛИКОМ. Итог: у звонка с более чем 500 чанками синтез с 501-го не оплачен, хотя точные беспотолочные объёмы лежат рядом в `usage["tts"]` и как частичное дополнение не используются никогда. Механизм отличается от CA1 (там несовпадение ставок) — здесь потеря объёма у источника. Ни §8.12, ни §8.14 не описывают, что происходит за потолком. Теста на сценарий >500 чанков нет. Найдено майнером G5/G6, проверено дирижёром лично |
| TA10 | **HIGH** | код (расширение поверхности V1b/TA3) | Поверхность V1b/TA3 шире, чем считалось: `CopilotRepository.get` (`storage/repos/copilot.py:243-246`) делает голый `s.get(CopilotConversationRow, conv_id)` **вообще без** org-проверки, и через него достижимы ещё два пути. (а) **Запись** от чужой организации: `POST /api/copilot/conversations/{id}/feedback` (`api/copilot.py:370-375`) зовёт `_load_conv(conv_id)` только ради 404 и затем `set_feedback` — чужой тенант ставит фидбек на переписке жертвы. (б) **Тривиализация эксплуатации**: шаринг-ссылка `?copilot=<id>` (`editor/src/App.tsx:67`) — штатный UI-механизм, который ведёт в тот же незащищённый `GET`; подбирать id не нужно, достаточно чтобы ссылка покинула организацию-владельца. Тестов на org-изоляцию copilot нет вообще. Не отдельный корень, а та же дыра — но чинить нужно в `repos/copilot.py`, иначе точечная заплатка в одном роуте оставит остальные. Найдено майнером G8, проверено дирижёром лично |
...
| N1 | LOW, **уточнён и сузился** | тест (приёмка по исходникам) | Уточнение 2026-08-03: у самой `resolve_health_port()` поведенческие тесты ЕСТЬ (`tests/test_ops_scaling.py:31-46`, три штуки) — непроверенным остаётся только стык с библиотекой. Второй агент независимо подтвердил, что `livekit-agents` в окружении не установлена (`pip show` пусто, `import livekit.agents` → ModuleNotFoundError), поэтому валидность kwarg `port` для пиненной `1.6.4` проверить нельзя нигде локально. **Остаётся открытым, нужен живой контур.** Исходный текст: `tests/test_ops_scaling.py:52-59` (`test_worker_options_receive_resolved_port`) читает ТЕКСТ `agent.py` и грепает `_worker_kwargs["port"]`; `WorkerOptions` не конструируется ни в одном тесте. Если в пиненной `livekit-agents 1.6.4` поле называется иначе, каждый воркер с заданным `CFLOW_AGENT_HEALTH_PORT` падает `TypeError` на старте, а `make test` остаётся зелёным. То же у `test_metrics_port_conflict_is_warned` (`:62-69`) — грепает текст на `log.warning`. Верификатор не смог подтвердить наличие поля (`livekit-agents` в окружении не установлена) — отсюда LOW, а не MEDIUM. **Проверить при живом окружении** |
| ✅ N2 | LOW, **ЗАКРЫТ** | пробел спеки | Закрыт 2026-08-03 требованием `monitoring.agent-metrics-port-conflict` (`agent.py:2297-2312`). Заодно опровергнута прежняя формулировка «отдаёт первый worker пула»: мультипроцессная агрегация дочерних джобов одного воркера и bind-конфликт HTTP-порта между воркерами пула — разные механизмы, при раздельных `PROMETHEUS_MULTIPROC_DIR` метрики процесса не отдаёт никто. `docs/DOCUMENTATION.md:5886` коду соответствует — расхождение было именно в спеке. Исходный текст: новая деградация «порт `/metrics` занят → при раздельных `PROMETHEUS_MULTIPROC_DIR` метрики этого процесса не отдаёт НИКТО» (`livekit_agent/agent.py:2298-2312`) не покрыта `monitoring/spec.md`: `metrics-endpoint` описывает порты и формат, `multiprocess-aggregation` — агрегацию, конфликт bind не описан нигде. При этом `monitoring/spec.md` уже помечена `verified b3db66a2` — пробел закрыт формально, но не содержательно |
| ✅ N3 | INFO, **ЗАКРЫТ** вместе с G9 | уточнение G9 | G9 якорит только `worker_load.py:61-88` (`resolve_health_port`); вторая половина — подключение к запуску, `WorkerOptions(port=…)` через `_worker_kwargs` (`agent.py:2321-2334`). Границы capability подтверждены: `voice-pipeline` — per-job рантайм, пул воркеров и health-порт вне её scope, так что G9 остаётся дырой МЕЖДУ capability, а не MISSING внутри voice-pipeline |
...
### Закрытие пробелов G1–G4: call-control (майнинг 2026-08-03, opus) — 11 → 30 Req + 3 Inv, 5 новых дефектов

Дефекты пришли от агента под именами T1–T5; **перенумерованы в CC1–CC5**, потому что T1/T2 в этом реестре уже заняты (anti-toll-fraud в telephony и кросс-тенантный TTS-кэш).

| # | Уровень | Тип | Суть |
|---|---|---|---|
| CC1 | MEDIUM | код (обработка ошибок) | **Уже переведённый звонок может получить фолбэк-реплику AI.** Конструирование `LiveKitAPI` для стопа egress стоит ВНЕ `try` (`livekit_agent/agent.py:1808-1814`, обращения `os.environ["LIVEKIT_URL"]`/`["LIVEKIT_API_KEY"]`/`["LIVEKIT_API_SECRET"]` — `KeyError` при отсутствии), а внешний `except Exception` диспетчера (`:2005-2008`) на ЛЮБОМ исключении после бриджа зовёт `_resume_transfer_failure`. Последствия: в комнату, где абонент уже говорит с человеком, уходит реплика AI; событие перевода перебивается `ok→error`; disposition `transferred_warm` теряется. Срабатывает только при `post_transfer_recording=false` и активном egress. Теста нет. Проверено дирижёром лично |
| CC2 | MEDIUM | код (утечка ресурса + гонка) | `_wait_bridge_end` (`livekit_agent/agent.py:1574-1596`) ждёт `await done.wait()` **без потолка**: джоб воркера и его слот `_worker_load` заняты весь пост-трансферный разговор людей, а `call_control_monitor` к этому моменту уже вышел (`while not ended_closer_started`), то есть `max_call_duration` эту фазу не страхует. Плюс узкая гонка в обратную сторону: `if identities <= present` — если участник ноги в этот миг не виден в `remote_participants`, ожидание пропускается целиком и `delete_room()` убивает только что сбриджованный звонок. Проверено дирижёром лично |
| CC3 | LOW (потеря данных) | код (асимметрия с ТЗ №83) | `auto_end_call` (`livekit_agent/agent.py:1182-1211`) взводит `ended_closer_started`/`finalized_at` ДО закрывающей фразы и не делает `commit_speech_tail`: реплика клиента поверх или после прощания в транскрипт не дописывается, остаётся только событием `post_finalize_speech`. В hangup-пути ТЗ №83 хвост коммитится — асимметрия двух путей завершения |
| CC4 | LOW | док↔код | §8.13 §3 говорит, что гейт брошенной трубки срабатывает, когда `transfer_tx` «в статусе pending/ok»; код (`agent.py:2138-2140`) проверяет `tx.get("status") != "error"`, то есть пропускает при ЛЮБОМ не-error статусе, включая отсутствующий. Сегодня эквивалентно (ставятся только pending/ok/error), но формулировка документа уже кода. В спеке написано по коду |
| CC5 | INFO | док↔код | `observer_incident` есть в `AUTO_END_DISPOSITIONS` (`engine/schema.py`), но отсутствует в `_CONNECT_REASONS` (`webhooks/payloads.py:35-45`): для легаси-сессии без canonical `status_reason` причина выводится не из disposition, а из `outcome`. Результат совпадает (`user_hangup`), но заявленная §8.8 «страховка по disposition» на этой причине не работает |

Тестовые дыры под новыми Requirement (кандидаты в волну тестов): hold и изоляция звука, наблюдение за бриджем, стоп egress на бридже, `three_way_seconds`, ожидание playout в диспетчере, ребайндинг рантайма при agent-хендоффе, transfer-гейт обработчика брошенной трубки.
