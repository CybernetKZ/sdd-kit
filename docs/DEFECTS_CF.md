...
| G7 | **ASR-часть ЗАКРЫТА** 2026-08-03 → 5 Req в `asr` (`picker-catalog-fetch`, `picker-client-filter`, `picker-inline-fallback`, `models-page-actions`, `models-page-check`); заодно сужен блок `## Out of Scope`, который раньше исключал весь UI-слой целиком. TTS-часть — за агентом `tts`. Исходный текст: UI-слои `AsrModelPicker/AsrModelsPage` (asr), `VoicesPage/VoicePicker` (tts) — не Requirement и не в исключениях | `editor/src/` | верификаторы asr, tts |
