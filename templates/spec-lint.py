...
VERIFIED_RE = re.compile(
    r"^>\s*Last verified:\s*(\S+)\s*\(commit\s+([0-9a-fA-F]{6,40})\)"
)
...
def check_deltas(root: Path) -> list[dict]:
    ...
    for path in sorted((root / "openspec" / "changes").glob("*/specs/*/spec.md")):
        ...
        unresolved = [
            a for a in sorted(set(spec["anchors"])) if not resolve_anchor(a, root)
        ]
...
def main() -> int:
    ...
    parser.add_argument(
        "--root", default=None, help="repository root (default: git root or cwd)"
    )
    parser.add_argument(
        "--strict", action="store_true", help="same as SPEC_LINT_STRICT=1"
    )
    parser.add_argument(
        "--json",
        action="store_true",
    ...
    if not specs:
        ...
        print(
            f"spec-lint: no specs under {root}/openspec/specs - nothing to check",
            file=sys.stderr,
        )
        return 0
        ...
        results.append(
            {
                "spec": rel,
                "requirements": spec["requirements"],
                "last_verified_date": spec["date"],
                "last_verified_commit": spec["commit"],
                "violations": spec["violations"],
                **freshness,
            }
        )
    ...
    print(
        f"spec-lint [{mode}] {len(results)} spec(s) checked: "
        + ", ".join(f"{k}={v}" for k, v in totals.items())
        + f"; metadata violations={violations}",
        file=out,
    )
    for item in results:
        print(
            f"  {item['status']:<10} {item['spec']}  "
            f"({item['requirements']} req, {item['enforced_files']} enforced file(s), "
            f"verified {item['last_verified_commit'] or '-'})",
            file=out,
        )
        if item["reason"]:
        ...
        for violation in item["violations"]:
            print(
                f"      {violation['kind']} (line {violation['line']}): {violation['message']}",
                file=out,
            )
    if deltas:
        bad = sum(1 for d in deltas if d["violations"] or d["unresolved_anchors"])
        print(
            f"spec-lint: {len(deltas)} change delta(s) checked, {bad} with findings",
            file=out,
        )
        for item in deltas:
            if not (item["violations"] or item["unresolved_anchors"]):
                continue
            print(
                f"  DELTA      {item['delta']}  ({item['requirements']} req)", file=out
            )
            for violation in item["violations"]:
                print(
                    f"      {violation['kind']} (line {violation['line']}): "
                    f"{violation['message']}",
                    file=out,
                )
            for anchor in item["unresolved_anchors"]:
                print(
                    f"      note: anchor does not resolve yet: {anchor} "
                    f"(expected only if this change creates it)",
                    file=out,
                )
    ...
    if not strict:
        print(
            "spec-lint: warn-only mode, exiting 0 (set SPEC_LINT_STRICT=1 to enforce)",
            file=out,
        )
        return 0
    if violations:
        print(
            "next: fix the metadata violations listed above, then re-run "
            "python3 .claude/scripts/spec-lint.py --strict",
            file=out,
        )
        return 1
    if totals["STALE"] or totals["MISSING"]:
        print(
            "next: re-verify each STALE/MISSING spec against the code and bump its "
            "'Last verified (commit)' line, then re-run",
            file=out,
        )
        return 2
    return 0
