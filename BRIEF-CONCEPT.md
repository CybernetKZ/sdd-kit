...
> Вторая сверка 2026-08-04 (ADR-0026) — «НЕ нужно усложнения» исполнен
> буквально: флаги выпилены подчистую, Makefile.sdd → scripts/sdd/*.sh,
> living-spec-check.sh удалён (main-drift.sh по требованию), store-ci.yml
> выпилен (CI без исключений). «PR + youtrack агентом» — только по явной
> команде разработчика, дефолт — человек. Профили deprecated-репо оставлены.
> Решено иначе (сверка 2026-08-05): «opus-оркестратор параллельных
> sonnet-имплементаторов» → один последовательный `executor` (sonnet),
> оркестратор = основная сессия; параллельные executors — позже, когда
> последовательный переживёт реальный тикет (ADR-0021).
