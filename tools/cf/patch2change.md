...
2. Номер 33 исторически использован ДВАЖДЫ, но файл патча существует один:
   `patch33-llm-model-directory.md` (справочник LLM-моделей, §7.1 документа).
   Второе использование - "события звонка в Истории" (§8.13) - живёт только
   записью в DOCUMENTATION.md, файла в `docs/patches/` у него нет.
   Конвертировать единственный файл как tz-033, в его proposal.md отметить,
   что номер задублирован с §8.13 и второй ТЗ №33 файла-источника не имеет.
