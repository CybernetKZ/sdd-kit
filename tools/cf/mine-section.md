...
4a. Утверждение без проверяемого сценария (гарантия-инвариант, в т.ч.
   негативная — «X никогда не попадает в Y») оформляется отдельной секцией
   `## Invariants` с блоками `### Invariant:` — НЕ `### Requirement:`
   (openspec validate считает Requirement вне `## Requirements` невидимым) и
   БЕЗ `#### Scenario:` (spec-lint это запрещает). Метаданные `id`/`enforced`
   допустимы. Негативную гарантию, которую держит отсутствие кода, так и
   пишем словами, ложный якорь не ставим.
5. Ничего не выдумывать: не описано в § и не видно в коде — не писать.
   ...
   на номер в `docs/DEFECTS_CF.md`) и внести в отчёт. НЕ использовать HTML-комментарии
